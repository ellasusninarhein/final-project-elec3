...
function setButtons() {
  const labelText = running ? "Pause" : (elapsed > 0 ? "Resume" : "Start");
  const iconChar = running ? "❚❚" : "▶";

  const labelEl = $startPause.querySelector(".label");
  const iconEl = $startPause.querySelector(".icon");

  if (labelEl) labelEl.textContent = labelText;
  if (iconEl) iconEl.textContent = iconChar;
  $startPause.setAttribute("aria-label", labelText);
